...
public class SocketChecker implements Runnable {
	 ...
	 */
	public static String checkSocket(String ahost, int aport, int timeout,
		...
		if (tcp) {
			...
			if (!error) { // if the socket is open Reader and Writer
				...
				if (!error) { // if Reader and Writer are open
					...
					try {
						response = "sending TCP=/" + message + "/; reply="
								+ is.readLine();
					} catch (IOException e) {
						response += "; " + e.toString();
					}
			...
			if (!error) {
				...
				try {
					dsock.send(new DatagramPacket(message.getBytes(), message
							.getBytes().length, InetAddress.getByName(server),
							port));
				...
				if (!error) {
					...
					try {
						...
						response = "server trying to overflow buffer: "
								+ e.toString();
						error = true;
					} catch (IOException e) {
						response = "IOException: " + e.toString();
